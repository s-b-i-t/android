...
// Shark fin animation
        binding.sharkFin.visibility = View.VISIBLE

        val sharkFinStart = -260f
        val sharkFinEnd = binding.sky.width.toFloat()
        val sharkFinAnimator = ObjectAnimator
            .ofFloat(binding.sharkFin, "translationX", sharkFinStart, sharkFinEnd)
            .setDuration(3000)
        sharkFinAnimator.interpolator = AccelerateInterpolator()

        val animatorSet = AnimatorSet()
        animatorSet.play(heightAnimator)
            .with(sunsetSkyAnimator)
            .before(nightSkyAnimator)
            .before(sharkFinAnimator)
        animatorSet.start()
    }
}